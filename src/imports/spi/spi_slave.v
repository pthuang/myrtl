...
module spi_slave # (
    ...
    output reg  [O_WIDTH-1:0]   o_rx_data   , // data payload 
    input                       mcs         , // 4-line spi Master interface 
    input                       sclk        , // 4-line spi Master interface 
    input                       mosi        , // 4-line spi Master interface 
    output reg                  miso          // 4-line spi Master interface 
); 
    ...
    always@(*) begin
        ...
        SLAVE_BUSY:
        begin
            if( cnt_mbusy == (SCK_DIV - 1) && cnt_bit == (O_WIDTH-1) && sclk == ~SCK_MODE[0]) begin
                n_state = SLAVE_OUT;
            end else begin
                n_state = SLAVE_BUSY;
            end
            ...
            begin
                ...
                if(cnt_mbusy == (SCK_DIV - 1)) begin
                    ...
                    if(sclk == ~SCK_MODE[0]) begin
                        cnt_bit <=  cnt_bit + 1;
                        if(cnt_bit ==  (O_WIDTH - 1)) begin
                            cnt_bit <=  0;
                        end
                    end else begin 
                        if(DATA_ENDIAN) begin
                            rx_data <=  {rx_data[O_WIDTH-2:0], mosi};
                            // rx_data[0]              <=  mosi;
                            // rx_data[O_WIDTH-1:1]    <=  rx_data[O_WIDTH-2:0];
                        end else begin
                            rx_data <=  {mosi, rx_data[O_WIDTH-1:1]};
                            // rx_data[O_WIDTH-1]      <=  mosi;
                            // rx_data[O_WIDTH-2:0]    <=  rx_data[O_WIDTH-1:1];
                        end
                    end
                end
            end
